...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            variableDomainsDict[obsVar] = [x for x in OBS_VALS]
            edges.append((FOOD_HOUSE_VAR, obsVar))
            edges.append((GHOST_HOUSE_VAR, obsVar))

    # Add the domain of X, Y, FH, and GH
    variableDomainsDict[X_POS_VAR] = X_POS_VALS
    variableDomainsDict[Y_POS_VAR] = Y_POS_VALS
    variableDomainsDict[FOOD_HOUSE_VAR] = [x for x in HOUSE_VALS]
    variableDomainsDict[GHOST_HOUSE_VAR] = [x for x in HOUSE_VALS]
